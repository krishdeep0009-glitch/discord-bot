...
def provision(vps_id, image, os_label, ram_mb, cpu_cores, disk_gb, cpu_name, gpu_name=None) -> tuple:
    ...
    ct_data = client.api.create_container(
        ...
        tty=True,
        stdin_open=True,
        environment={"TERM": "xterm-256color", "container": "docker", "HOSTNAME": vps_id},
        command="/sbin/init",
    ...
    # ── Step 7b: Fake /etc/os-release — Ubuntu-style DracoHost branding ──
    # Codename map, mimicking Ubuntu's "24.04.4 LTS (Noble Numbat)" style
    codename_map = {
        "Ubuntu 20.04": ("20.04.6 LTS", "Focal Fossa"),
        "Ubuntu 22.04": ("22.04.5 LTS", "Jammy Jellyfish"),
        "Ubuntu 24.04": ("24.04.4 LTS", "Noble Numbat"),
        "Debian 11":    ("11.11",       "Bullseye"),
        "Debian 12":    ("12.9",        "Bookworm"),
    }
    version_full, codename = codename_map.get(os_label, (os_label, "DracoHost"))

    write_file(ct, "/etc/dracohost/os-release",
        'NAME="DracoHost"\n'
        f'VERSION="{version_full} ({codename})"\n'
        'ID=dracohost\n'
        'ID_LIKE=debian\n'
        f'PRETTY_NAME="DracoHost {version_full} ({codename})"\n'
        f'VERSION_ID="{version_full.split()[0]}"\n'
        f'VERSION_CODENAME={codename.split()[0].lower()}\n'
    ...
    log.info(f"[{vps_id}] os-release faked — DracoHost {version_full} ({codename})")
    ...
    # ── Step 7d: Optional GPU line (only if admin provided one) ──────
...
@app_commands.describe(user="Target user", ram="RAM in MB", cpu="CPU cores",
    ...
    suspend_in_days="Days until auto-suspend (0=never)",
    gpu="Optional — fake GPU shown in neofetch (e.g. 'NVIDIA RTX 5070')")
...
async def cmd_create(ix: discord.Interaction, user: discord.Member, ram: int, cpu: float,
    ...
    suspend_in_days: int = 0, gpu: str = None):
    await ix.response.defer(ephemeral=True)
    if not is_admin(ix): return await ix.followup.send(embed=em("⛔ Forbidden", "Admin only.", RED))
    ...
    await do_create(ix, user, ram, cpu, disk, os_val, cpu_val, suspend_in_days, gpu)
...
class DeployModal(discord.ui.Modal, title="🐉 DracoHost — Deploy VPS"):
    ...
    gpu  = discord.ui.TextInput(label="GPU (optional)", placeholder="e.g. NVIDIA RTX 5070", required=False, max_length=60)
