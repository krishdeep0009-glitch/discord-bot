...
- neofetch shows exact assigned RAM, CPU, custom CPU name
...
# ─────────────────────────────────────────────
# Build fake /proc/meminfo content
# ─────────────────────────────────────────────
...
def make_fake_meminfo(memory_mb: int) -> str:
    ...
    cached_kb = int(kb * 0.05)
    active_kb = int(kb * 0.10)
    anon_kb   = int(kb * 0.08)
    mapped_kb = int(kb * 0.02)
    return "\n".join([
        f"MemTotal:       {kb} kB",
        ...
        f"Dirty:                4 kB",
        f"Writeback:            0 kB",
        f"AnonPages:      {anon_kb} kB",
        f"Mapped:         {mapped_kb} kB",
        f"Shmem:               64 kB",
        f"Slab:               512 kB",
...
# ─────────────────────────────────────────────
# Build fake /proc/cpuinfo content
# ─────────────────────────────────────────────
...
def provision_vps(vps_id: str, image: str, memory_mb: int,
    ...
    """
    1. Start container (--privileged for bind-mount, exact RAM/CPU limits)
    2. apt update
    3. apt install -y tmate neofetch
    4. Write /etc/blined/meminfo  → bind-mount over /proc/meminfo
    5. Write /etc/blined/cpuinfo  → bind-mount over /proc/cpuinfo
    6. Write /etc/rc.local to re-apply on restart
    7. Set hostname, MOTD
    8. Start tmate, return (container, ssh_cmd, web_url)
    """
    client = get_docker()
    ...
    log.info(f"[{vps_id}] Creating container — RAM:{memory_mb}MB CPU:{cpu_cores} Disk:{disk_gb}GB")

    # ── Create container ────────────────────────────────────────────
    try:
        container = client.containers.run(
            image, name=vps_id, detach=True,
            privileged=True,
            mem_limit=mem_str, memswap_limit=mem_str,
    ...
    # ── Create /etc/blined dir ──────────────────────────────────────
    container.exec_run("mkdir -p /etc/blined", tty=False)

    # ── Write and bind-mount fake /proc/meminfo ─────────────────────
    fake_mem = make_fake_meminfo(memory_mb)
    container_put(container, "/etc/blined/meminfo", fake_mem)
    container.exec_run(
        "bash -c 'mount --bind /etc/blined/meminfo /proc/meminfo'",
        tty=False,
    )
    log.info(f"[{vps_id}] /proc/meminfo bind-mounted ({memory_mb} MB).")

    # ── Write and bind-mount fake /proc/cpuinfo ─────────────────────
    fake_cpu = make_fake_cpuinfo(cpu_cores, cpu_name)
    container_put(container, "/etc/blined/cpuinfo", fake_cpu)
    container.exec_run(
        "bash -c 'mount --bind /etc/blined/cpuinfo /proc/cpuinfo'",
        tty=False,
    )
    log.info(f"[{vps_id}] /proc/cpuinfo bind-mounted ({int(cpu_cores)} vCPU, name: {cpu_name}).")

    # ── /etc/rc.local — re-apply mounts on restart ──────────────────
    rc_local = (
        "#!/bin/bash\n"
    ...
    container.exec_run(
        f"bash -c 'hostname {vps_id} && echo {vps_id} > /etc/hostname'",
        tty=False,
    )
    motd = (
        f"\n"
        ...
        f"  ╠══════════════════════════════╣\n"
        f"  ║  VPS ID : {vps_id:<19}║\n"
        f"  ║  RAM    : {str(memory_mb)+' MB':<19}║\n"
        f"  ║  CPU    : {str(cpu_int)+' vCore(s)':<19}║\n"
        f"  ║  Disk   : {str(disk_gb)+' GB':<19}║\n"
        f"  ║  OS     : {image:<19}║\n"
        f"  ╚══════════════════════════════╝\n\n"
    )
...
async def cmd_reinstall(interaction: discord.Interaction, vps_id: str):
    ...
    with db_connect() as c:
        row = c.execute("SELECT * FROM vps_instances WHERE vps_id=?", (vps_id,)).fetchone()
    await interaction.followup.send(embed=embed("⏳ Reinstalling...", f"Running apt update → apt install tmate neofetch → tmate\n~60 seconds...", WARN_COLOR))
    try:
        try:
        ...
        container, ssh_cmd, web_url = await asyncio.get_event_loop().run_in_executor(
            None, lambda: provision_vps(vps_id, row["os_image"], row["memory_mb"], row["cpu_cores"], row["disk_gb"])
        )
        with db_connect() as c:
...
async def cmd_my_vps(interaction: discord.Interaction):
    ...
    fields = [(r["vps_id"], f"OS: `{r['os_image']}` | RAM: `{r['memory_mb']}MB` | CPU: `{r['cpu_cores']}` | Disk: `{r['disk_gb']}GB` | Status: `{r['status']}`", False) for r in rows]
...
async def cmd_commands(interaction: discord.Interaction):
    ...
    r = embed("📖 Reference", "", INFO_COLOR, fields=[
        ("OS Templates", "`ubuntu22` → Ubuntu 22.04\n`debian11` → Debian 11", False),
        ("VPS ID Format", "Auto-assigned: `BC-0001`, `BC-0002` ...", False),
        ("Credentials", "SSH/tmate links sent via **DM only** — never public.", False),
        ("CPU Name", "Set a custom CPU name in `/create` shown in `neofetch`.", False),
    ])
...
@app_commands.describe(
    ...
    os="OS: ubuntu22 | debian11",
    cpu_name="Custom CPU name shown in neofetch (e.g. 'Intel Xeon E5-2680')",
)
async def cmd_create(
    ...
    disk: int,
    os: str,
    cpu_name: str = "Blined Cloud Virtual CPU",
):
    await interaction.response.defer(ephemeral=True)
    if not is_admin(interaction):
        return await interaction.followup.send(embed=embed("⛔ Forbidden", "Admin only.", ERROR_COLOR))
    ...
    if os_lower not in OS_MAP:
        return await interaction.followup.send(embed=embed("❌ Invalid OS", f"Use: {', '.join(OS_MAP.keys())}", ERROR_COLOR))
    ...
    await interaction.followup.send(embed=embed(
        ...
        f"[3/4] Faking /proc/meminfo+cpuinfo ⏳\n"
        f"[4/4] Starting tmate             ⏳\n```\n"
        f"~60 seconds... Credentials will be sent to user's DM.",
        INFO_COLOR,
        fields=[
            ("OS",  image,            True),
            ...
            ("Disk",f"{disk} GB",     True),
            ("CPU Name", cpu_name,    False),
        ],
    ))

    try:
        container, ssh_cmd, web_url = await asyncio.get_event_loop().run_in_executor(
            None, lambda: provision_vps(vps_id, image, memory, cpu, disk, cpu_name)
        )
    except Exception as e:
    ...
    with db_connect() as c:
        c.execute("""
            INSERT INTO vps_instances
              (vps_id, user_id, container_id, os_image, memory_mb, cpu_cores, disk_gb, tmate_ssh, tmate_web, status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'running')
        """, (vps_id, user.id, container.id, image, memory, cpu, disk, ssh_cmd, web_url))
    ...
    try:
        ...
        await dm.send(embed=embed(
            ...
            fields=[
                ...
                ("🖥 OS",         image,                                                        True),
                ("🧠 RAM",        f"{memory} MB",                                               True),
                ("💻 CPU",        f"{cpu} core(s) — {cpu_name}",                               True),
                ("💾 Disk",       f"{disk} GB",                                                 True),
    ...
    note = "✅ Credentials sent to DM." if dm_sent else "⚠️ Could not DM user — share credentials manually."
    await interaction.followup.send(embed=embed(
        "✅ VPS Created",
    ...
    # Public channel ping — no credentials
...
async def cmd_list_vps(interaction: discord.Interaction):
    ...
    fields = [(r["vps_id"], f"<@{r['user_id']}> | OS:`{r['os_image']}` RAM:`{r['memory_mb']}MB` CPU:`{r['cpu_cores']}` Disk:`{r['disk_gb']}GB` Status:`{r['status']}`", False) for r in rows]
