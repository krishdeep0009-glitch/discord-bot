...
#
# IMPORTANT: these are CUSTOM images (built via blinedcloud-images/build_images.sh),
# NOT the stock Docker Hub ubuntu:/debian: images. Stock images are minimal and do
# NOT include systemd, so /sbin/init does not exist in them — that caused container
# start to fail with "exec: /sbin/init: stat /sbin/init: no such file or directory".
# These custom images have systemd + tmate + neofetch pre-installed.
OS_MAP = {
    "ubuntu20": "blinedcloud/ubuntu20-systemd",
    "ubuntu22": "blinedcloud/ubuntu22-systemd",
    "ubuntu24": "blinedcloud/ubuntu24-systemd",
    "debian11": "blinedcloud/debian11-systemd",
    "debian12": "blinedcloud/debian12-systemd",
}
...
def provision_vps(vps_id: str, image: str, memory_mb: int,
    ...
    try:
        try:
            old = client.containers.get(vps_id)
    ...
    def _is_name_conflict(exc) -> bool:
        return isinstance(exc, docker.errors.APIError) and getattr(exc, "status_code", None) == 409

    def _force_clear_name(reason: str):
        """Force-remove anything holding this container name, however it's found."""
        log.warning(f"[{vps_id}] {reason} — forcing removal of '{vps_id}' before retry.")
        try:
            stray = client.containers.get(vps_id)
            stray.remove(force=True, v=True)
        except docker.errors.NotFound:
            pass
        except Exception as rm_err:
            log.error(f"[{vps_id}] Force-remove during retry failed: {rm_err}")
        # also try raw low-level API removal in case the high-level model is stale
        try:
            client.api.remove_container(vps_id, force=True, v=True)
        except Exception:
            pass
        time.sleep(1.5)
        ...
        for attempt in range(5):
            try:
                ...
                if _is_name_conflict(e):
                    _force_clear_name(f"Name conflict on attempt {attempt+1}/5")
                    continue
                raise
        raise last_err
    ...
    try:
        ...
        if _is_name_conflict(e):
            # Genuinely couldn't clear the name after 5 attempts — surface clearly.
            raise RuntimeError(
                f"Container name '{vps_id}' is stuck and could not be cleared after 5 attempts "
                f"(last error: {e}). Check 'docker ps -a' on the host for a container in a "
                f"'Removing' or zombie state, or confirm no second bot process is running."
            ) from e
    ...
    # ── Ensure tmate + neofetch present ───────────────────────────────
    # These are baked into the custom blinedcloud/*-systemd images already
    # (see blinedcloud-images/build_images.sh), so this is just a fast
    # safety-net check, not a full apt update+install on every /create.
    check = container.exec_run("bash -c 'command -v tmate'", tty=False)
    if check.exit_code != 0:
        log.warning(f"[{vps_id}] tmate not found in image — falling back to runtime install (this image wasn't built with build_images.sh).")
...
# Serializes VPS ID assignment + provisioning so concurrent /create calls
# (double-clicks, redelivered interactions, multiple admins at once) can't
# race onto the same vps_id and trigger Docker 409 name conflicts.
