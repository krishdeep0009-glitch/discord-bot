...
def provision(vps_id, image, memory_mb, cpu_cores, disk_gb, cpu_name) -> tuple:
    ...
    ✅ cap_add SYS_ADMIN   — needed for /proc bind mounts (no --privileged)
    ...
    # ── Create container ────────────────────────────────────────────
    # ── Pull systemd-ready image ────────────────────────────────────
    # We use jrei/systemd images which work with cgroupv2 properly
    # by setting cgroupns_mode="host" — this avoids the threaded mode error
    # while still allowing systemd to run as PID 1 and systemctl to work.
    systemd_image_map = {
    ...
    systemd_image = systemd_image_map.get(image, image)
    log.info(f"[{vps_id}] Using systemd image: {systemd_image}")

    # ── Create container via low-level API to pass CgroupnsMode=host ──
    # The high-level docker-py client doesn't support cgroupns_mode in older
    # versions, so we use client.api.create_container() directly which maps
    # 1-to-1 with the Docker Engine API and supports all parameters.
    #
    # CgroupnsMode="host" is the fix for:
    # "cannot enter cgroupv2 in threaded mode"
    # It shares the host cgroup namespace so systemd can manage cgroups
    # without hitting the threaded-mode restriction.
    ...
    host_config_kwargs = dict(
        ...
        cpu_quota=quota,
        privileged=True,
        cgroup_ns_mode="host",          # <-- low-level API param name
        binds={
    ...
    # Try with disk quota first
    try:
        host_config_kwargs["storage_opt"] = {"size": f"{disk_gb}G"}
        host_cfg = client.api.create_host_config(**host_config_kwargs)
    except Exception:
        host_config_kwargs.pop("storage_opt", None)
        host_cfg = client.api.create_host_config(**host_config_kwargs)

    try:
        ct_resp = client.api.create_container(
            image=systemd_image,
            name=vps_id,
            detach=True,
            tty=True,
        ...
        client.api.start(ct_resp["Id"])
        ct = client.containers.get(ct_resp["Id"])
        log.info(f"[{vps_id}] Container created with cgroupns=host + systemd.")
    except Exception as e:
        # Final fallback — no cgroup namespace setting, no disk quota
        log.warning(f"[{vps_id}] Low-level create failed ({e}), trying minimal fallback.")
        ...
        client.api.start(ct_resp["Id"])
        ct = client.containers.get(ct_resp["Id"])
        log.warning(f"[{vps_id}] Running in fallback mode (tail -f /dev/null).")

    # Wait for systemd/container to initialize
    time.sleep(6)
    ...
    # ── apt install ─────────────────────────────────────────────────
    log.info(f"[{vps_id}] Installing tmate, neofetch and core tools...")
    ct.exec_run(
        "bash -c 'DEBIAN_FRONTEND=noninteractive apt-get install -y -qq "
        "tmate neofetch curl wget sudo procps net-tools iproute2'",
        tty=False,
    )
