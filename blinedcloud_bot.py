"""
Blined Cloud VPS Manager
━━━━━━━━━━━━━━━━━━━━━━━
• Docker containers as VPS (jrei/systemd images — systemctl works)
...
C_YELLOW = 0xFEE75C
C_BLUE   = 0x3498DB
FOOTER   = "Powered by Blined Cloud"
...
def next_vps_id() -> str:
    ...
    dk_max = 0
    try:
        for ct in dclient().containers.list(all=True, filters={"label":"managed-by=blined-cloud"}):
            if ct.name.startswith("blinedcloud-vps-"):
                try: dk_max = max(dk_max, int(ct.name.split("-")[-1]))
                except: pass
    except: pass
    return f"blinedcloud-vps-{max(db_num, dk_max+1):04d}"
...
def provision(vps_id, image, memory_mb, cpu_cores, disk_gb, cpu_name) -> tuple:
    ...
    ✅ jrei/systemd image  — systemd PID 1, systemctl fully works
    ✅ --privileged        — cgroup management + /proc bind mounts
    ...
    # ── Fake /proc/meminfo ──────────────────────────────────────────
    ct.exec_run("mkdir -p /etc/blined", tty=False)
    put(ct, "/etc/blined/meminfo", fake_mem(memory_mb))
    ct.exec_run("bash -c 'mount --bind /etc/blined/meminfo /proc/meminfo'", tty=False)
    log.info(f"[{vps_id}] /proc/meminfo faked ({memory_mb} MB)")

    # ── Fake /proc/cpuinfo ──────────────────────────────────────────
    put(ct, "/etc/blined/cpuinfo", fake_cpu(cpu_cores, cpu_name))
    ct.exec_run("bash -c 'mount --bind /etc/blined/cpuinfo /proc/cpuinfo'", tty=False)
...
class Bot(commands.Bot):
    ...
    async def on_ready(self):
        ...
        await self.change_presence(activity=discord.Activity(
            type=discord.ActivityType.watching, name="Powered by Blined Cloud"))
...
@app_commands.describe(vps_id="e.g. blinedcloud-vps-0001")
...
async def cmd_help(ix: discord.Interaction):
    ...
    r = em("📖 Reference","",C_BLUE,fields=[
        ...
        ("VPS ID Format",   "`blinedcloud-vps-0001`, `blinedcloud-vps-0002` ...",                 False),
        ("Terminal Access", "SSH via tmate only — sent to DM, never public",                      False),
        ("systemctl",       "Full systemd support — `systemctl`, services, cron all work",        False),
