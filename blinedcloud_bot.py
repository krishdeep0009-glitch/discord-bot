...
def next_vps_id():
    ...
    return "blinedcloud-vps-0001" if not row else f"blinedcloud-vps-{int(row['vps_id'].split('-')[-1])+1:04d}"
...
def provision_vps(vps_id: str, image: str, memory_mb: int,
    ...
    # (e.g. tmate timed out) and left the container behind. Docker refuses
    # to create a new container with a name already in use, so we clean
    # up before trying.
    try:
        old = client.containers.get(vps_id)
        log.warning(f"[{vps_id}] Found leftover container ({old.short_id}) — removing it first.")
        old.remove(force=True)
    except docker.errors.NotFound:
        pass
    except Exception as e:
        log.warning(f"[{vps_id}] Could not remove leftover container: {e}")
...
async def cmd_create(
    ...
    try:
        ...
        return await interaction.followup.send(embed=embed(
            "❌ Provisioning Failed",
            f"Could not create **{vps_id}**.\n```{str(e)[:500]}```\nAny partial container has been cleaned up — you can try `/create` again.",
            ERROR_COLOR,
        ))
