...
def provision_vps(vps_id: str, image: str, memory_mb: int,
    ...
    """
    Creates a container using standard ubuntu/debian images:
      - tail -f /dev/null as PID 1 (simple, works on all images)
      - --privileged for bind-mounting fake /proc/meminfo + /proc/cpuinfo
      - Exact RAM / CPU / Disk limits
      - apt update -> apt install tmate neofetch
      - Fake /proc/meminfo + /proc/cpuinfo via bind-mount
    ...
    # We use "tail -f /dev/null" as PID 1 — simple, reliable, works on
    # ALL standard ubuntu/debian images with zero extra setup.
    # --privileged is kept ONLY for the /proc bind-mount trick (fake RAM/CPU).
    # tty=True gives users a proper terminal experience via tmate.
    run_kwargs = dict(
        name=vps_id,
        detach=True,
        privileged=True,     # needed for mount --bind /etc/blined/meminfo /proc/meminfo
        tty=True,
        stdin_open=True,
        mem_limit=mem_str,
        ...
        command="tail -f /dev/null",   # keeps container alive; tmate is started via exec
    ...
    # Small wait to make sure the container is fully up before exec
    time.sleep(2)
...
async def cmd_commands(interaction: discord.Interaction):
    ...
    r = embed("📖 Reference", "", INFO_COLOR, fields=[
        ...
        ("Container Type", "Docker container with `tail -f /dev/null`, `--privileged` for fake /proc mounts.", False),
