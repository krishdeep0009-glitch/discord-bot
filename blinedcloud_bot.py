...
def provision(vps_id, image, memory_mb, cpu_cores, disk_gb, cpu_name) -> tuple:
    ...
    kw = dict(
        ...
        tty=True,
        stdin_open=True,
        # cgroupns_mode="host" is the KEY fix:
        # It lets the container use the HOST cgroup namespace instead of
        # creating a new one — this avoids the "threaded mode" error on
        # cgroup v2 hosts while allowing systemd to manage cgroups normally.
        cgroupns_mode="host",
        privileged=True,
        mem_limit=mem,
        memswap_limit=mem,
        ...
        command="/sbin/init",           # systemd as PID 1 — systemctl works
        tmpfs={
            "/run":      "rw,nosuid,nodev",
    ...
    try:
        ...
        ct = client.containers.run(systemd_image, **kw)
        log.info(f"[{vps_id}] Created with disk quota and systemd.")
    except Exception as e:
