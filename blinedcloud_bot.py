...
# Systemd-enabled images — support systemctl, services, cron etc.
# jrei/systemd-* images are pre-configured for systemd as PID 1 inside Docker.
OS_MAP = {
    "ubuntu20": "jrei/systemd-ubuntu:20.04",
    "ubuntu22": "jrei/systemd-ubuntu:22.04",
    "ubuntu24": "jrei/systemd-ubuntu:24.04",
    "debian11": "jrei/systemd-debian:11",
    "debian12": "jrei/systemd-debian:12",
}

# Human-readable OS names (for embeds)
OS_NAMES = {
...
def provision_vps(vps_id: str, image: str, memory_mb: int,
    ...
    """
    Creates a proper LXC-style container with systemd as PID 1:
      - Uses jrei/systemd-ubuntu or jrei/systemd-debian images
      - systemd as PID 1 (/sbin/init) — systemctl fully works
      - --privileged for cgroup management + /proc bind mounts
      - Exact RAM / CPU / Disk limits enforced
      - apt update -> apt install tmate neofetch
      - Fake /proc/meminfo + /proc/cpuinfo via bind-mount (neofetch shows correct specs)
      - Start tmate session, return (container, ssh_cmd, web_url)
    """
    client = get_docker()
    ...
    # ── Container run config — LXC-style with systemd as PID 1 ────
    # Uses jrei/systemd-* images which are pre-configured for systemd
    # inside Docker. This gives a real VPS experience:
    #   - systemctl start/stop/enable/disable works
    #   - cron, ssh, nginx, apache etc. all start as services
    #   - journalctl logs work
    #   - /run, /run/lock, /tmp as tmpfs (same as real LXC)
    # --privileged needed for:
    #   1. systemd to manage cgroups
    #   2. mount --bind fake /proc/meminfo + /proc/cpuinfo
    run_kwargs = dict(
        name=vps_id,
        detach=True,
        privileged=True,            # required for systemd cgroup management + /proc bind mounts
        tty=True,
        stdin_open=True,
        mem_limit=mem_str,
        ...
        environment={
            "TERM": "xterm-256color",
            "container": "docker",  # tells systemd it's running inside Docker
        },
    ...
    try:
        ...
        log.warning(f"[{vps_id}] storage_opt failed ({e}), retrying without disk cap.")
        run_kwargs.pop("storage_opt", None)
        container = client.containers.run(image, **run_kwargs)
        log.info(f"[{vps_id}] Container created without disk quota.")

    # Wait for systemd to fully boot inside the container
    # systemd takes a few seconds to reach its default target
    log.info(f"[{vps_id}] Waiting for systemd to boot...")
    time.sleep(6)
...
async def cmd_commands(interaction: discord.Interaction):
    ...
    r = embed("📖 Reference", "", INFO_COLOR, fields=[
        ...
        ("Container Type", "LXC-style — systemd as PID 1, `systemctl` works, uses `jrei/systemd-*` images.", False),
...
async def cmd_create(
    ...
    await interaction.followup.send(embed=embed(
        ...
        f"```\n[1/3] apt update                   ⏳\n"
        f"[2/3] apt install tmate neofetch   ⏳\n"
        f"[3/3] Starting tmate               ⏳\n```\n"
...
# ══════════════════════════════════════════════
# 1-CLICK DEPLOY — Button Panel + Modal
# ══════════════════════════════════════════════

# Stores pending deploy state per admin user between button click and modal submit
# { user_id: {"os": ..., "cpu_name": ..., "target_user": discord.Member} }
_deploy_state: dict = {}


class DeployModal(discord.ui.Modal, title="🌩️ Blined Cloud — Deploy VPS"):
    """Modal form that pops up after the admin selects OS + CPU from buttons."""
    ...
    suspend_days = discord.ui.TextInput(
    ...
    def __init__(self, target_user: discord.Member, os_key: str, cpu_key: str):
        super().__init__()
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Validate inputs
        try:
            memory_mb    = int(self.memory.value.strip())
            cpu_cores    = float(self.cpu.value.strip())
            disk_gb      = int(self.disk.value.strip())
            suspend_days = int(self.suspend_days.value.strip())
        except ValueError:
            return await interaction.followup.send(embed=embed(
                "❌ Invalid Input",
                "RAM, CPU, Disk, and Days must be numbers.\nExample: RAM=512, CPU=1, Disk=10, Days=30",
                ERROR_COLOR,
            ))

        image         = OS_MAP[self.os_key]
        cpu_full_name = CPU_TYPES[self.cpu_key]
        os_name       = OS_NAMES.get(self.os_key, self.os_key)
        ...
        suspend_note = "Never expires"
        if suspend_days > 0:
            suspend_dt   = datetime.datetime.utcnow() + datetime.timedelta(days=suspend_days)
            suspend_at   = suspend_dt.isoformat()
            suspend_note = f"Auto-suspends <t:{int(suspend_dt.timestamp())}:R>"

        await interaction.followup.send(embed=embed(
            "⏳ Provisioning...",
            f"**{vps_id}** for {self.target_user.mention}\n\n"
            f"```\n[1/3] apt update                   ⏳\n"
            f"[2/3] apt install tmate neofetch   ⏳\n"
            f"[3/3] Starting tmate               ⏳\n```\n"
            f"~60 seconds... Credentials sent to user's DM.",
            ...
            fields=[
                ("OS",        os_name,              True),
                ("RAM",       f"{memory_mb} MB",    True),
                ("CPU",       f"{cpu_cores} core(s)", True),
        ...
        try:
            container, ssh_cmd, web_url = await asyncio.get_event_loop().run_in_executor(
                None, lambda: provision_vps(vps_id, image, memory_mb, cpu_cores, disk_gb, cpu_full_name)
            )
        except Exception as e:
            log.error(f"deploy modal create failed for {vps_id}: {e}")
            try:
                get_docker().containers.get(vps_id).remove(force=True, v=True)
            except Exception:
                pass
            return await interaction.followup.send(embed=embed(
                "❌ Provisioning Failed",
                f"Could not create **{vps_id}**.\n```{str(e)[:500]}```\nRun `/fix-vps {vps_id}` then try again.",
                ERROR_COLOR,
            ))
        ...
        with db_connect() as c:
            c.execute("""
                INSERT INTO vps_instances
                  (vps_id, user_id, container_id, os_image, memory_mb, cpu_cores, disk_gb, cpu_name, tmate_ssh, tmate_web, status, suspend_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'running', ?)
            """, (vps_id, self.target_user.id, container.id, image, memory_mb, cpu_cores,
                  disk_gb, cpu_full_name, ssh_cmd, web_url, suspend_at))

        log.info(f"{interaction.user} deployed {vps_id} for {self.target_user} via /deploy.")
        ...
        try:
            dm_fields = [
                ...
                ("🖥 OS",     os_name,                                                         True),
                ("🧠 RAM",    f"{memory_mb} MB",                                               True),
                ("💻 CPU",    f"{cpu_cores} core(s) — {cpu_full_name}",                       True),
                ("💾 Disk",   f"{disk_gb} GB",                                                 True),
            ]
            if suspend_at:
                dm_fields.append(("⏰ Expiry", suspend_note, True))
            ...
            dm = await self.target_user.create_dm()
            await dm.send(embed=embed(
                "🎉 Your VPS is Ready — Blined Cloud",
                "⚠️ **Keep these private — anyone with the link can access your terminal.**",
                SUCCESS_COLOR, fields=dm_fields,
            ))
            dm_sent = True
        except discord.Forbidden:
            log.warning(f"Could not DM {self.target_user}.")

        note = "✅ Credentials sent to DM." if dm_sent else "⚠️ Could not DM user — share manually."
        await interaction.followup.send(embed=embed(
            "✅ VPS Deployed",
            f"**{vps_id}** is live for {self.target_user.mention}.\n{note}",
            SUCCESS_COLOR,
            fields=[
                ("🆔 VPS ID",    vps_id,               True),
                ("👤 Owner",     str(self.target_user), True),
                ("🖥 OS",        os_name,               True),
                ("🧠 RAM",       f"{memory_mb} MB",     True),
                ("💻 CPU",       f"{cpu_cores} core(s)", True),
                ("💾 Disk",      f"{disk_gb} GB",        True),
        ...
        if interaction.channel:
            await interaction.channel.send(embed=embed(
                "🌩️ VPS Provisioned",
                f"{self.target_user.mention} — your **{vps_id}** is ready! Check your **DMs** for the tmate link.",
                BRAND_COLOR,
                fields=[
                    ("🆔 VPS ID", vps_id,                True),
                    ("🖥 OS",     os_name,                True),
...
class OSSelectView(discord.ui.View):
    ...
    async def _pick_os(self, interaction: discord.Interaction, os_key: str):
        if not is_admin(interaction):
            return await interaction.response.send_message(embed=embed("⛔ Forbidden", "Admin only.", ERROR_COLOR), ephemeral=True)
        # Move to Step 2 — pick CPU
        await interaction.response.edit_message(
            embed=embed(
                "🌩️ Deploy VPS — Step 2/3",
                f"**OS selected:** {OS_NAMES.get(os_key, os_key)}\n\nNow choose the **CPU type**:",
                BRAND_COLOR,
            ),
            view=CPUSelectView(self.target_user, os_key),
        )

    @discord.ui.button(label="Ubuntu 20.04", style=discord.ButtonStyle.secondary, emoji="🐧", row=0)
    async def ubuntu20(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self._pick_os(interaction, "ubuntu20")
    ...
    async def ubuntu22(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self._pick_os(interaction, "ubuntu22")
    ...
    async def ubuntu24(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self._pick_os(interaction, "ubuntu24")

    @discord.ui.button(label="Debian 11", style=discord.ButtonStyle.secondary, emoji="🌀", row=1)
    async def debian11(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self._pick_os(interaction, "debian11")

    @discord.ui.button(label="Debian 12", style=discord.ButtonStyle.primary, emoji="🌀", row=1)
    async def debian12(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self._pick_os(interaction, "debian12")

    @discord.ui.button(label="Cancel", style=discord.ButtonStyle.danger, emoji="✖️", row=1)
    async def cancel(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.edit_message(
            embed=embed("❌ Cancelled", "Deployment cancelled.", ERROR_COLOR), view=None)


class CPUSelectView(discord.ui.View):
    """Step 2 — Select CPU type."""
    def __init__(self, target_user: discord.Member, os_key: str):
        super().__init__(timeout=120)
        self.target_user = target_user
        self.os_key      = os_key

    async def _pick_cpu(self, interaction: discord.Interaction, cpu_key: str):
        if not is_admin(interaction):
            return await interaction.response.send_message(embed=embed("⛔ Forbidden", "Admin only.", ERROR_COLOR), ephemeral=True)
        # Step 3 — open modal for RAM/CPU/Disk/Days
        await interaction.response.send_modal(
            DeployModal(self.target_user, self.os_key, cpu_key)
        )

    @discord.ui.button(label="AMD Ryzen 9 9950X", style=discord.ButtonStyle.danger, emoji="🔴", row=0)
    async def ryzen9(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self._pick_cpu(interaction, "ryzen9")

    @discord.ui.button(label="Intel Xeon Platinum 8480+", style=discord.ButtonStyle.primary, emoji="🔵", row=0)
    async def xeon(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self._pick_cpu(interaction, "xeon")

    @discord.ui.button(label="← Back", style=discord.ButtonStyle.secondary, emoji="◀️", row=1)
    async def back(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.edit_message(
            embed=embed(
                "🌩️ Deploy VPS — Step 1/3",
                f"Deploying for **{self.target_user.display_name}**\n\nChoose the **Operating System**:",
                BRAND_COLOR,
            ),
            view=OSSelectView(self.target_user),
        )
...
@bot.tree.command(name="deploy", description="[Admin] 1-click VPS deploy — button panel for OS/CPU, form for specs.")
@app_commands.describe(user="The Discord user to deploy a VPS for")
async def cmd_deploy(interaction: discord.Interaction, user: discord.Member):
    if not is_admin(interaction):
        return await interaction.response.send_message(
            embed=embed("⛔ Forbidden", "Admin only.", ERROR_COLOR), ephemeral=True)
