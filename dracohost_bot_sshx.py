...
import os, io, time, random, tarfile, asyncio, logging, sqlite3, datetime, re
...
def provision(vps_id, image, os_label, ram_mb, cpu_cores, disk_gb, cpu_name, gpu_name=None) -> tuple:
    ...
    log.info(f"[{vps_id}] Installing neofetch, SSHX, tools...")
    ct.exec_run(
        "bash -c 'DEBIAN_FRONTEND=noninteractive apt-get install -y -qq "
        "neofetch curl wget sudo procps net-tools iproute2 htop'",
        tty=False,
    )

    # Install SSHX from the official installer.
    log.info(f"[{vps_id}] Installing SSHX client...")
    sshx_install = ct.exec_run(
        "bash -c 'curl -sSf https://sshx.io/get | sh'",
        tty=False,
    )
    if sshx_install.exit_code != 0:
    ...
    # ── Step 9: SSHX web terminal session ───────────────────────────
    log.info(f"[{vps_id}] Starting SSHX web terminal...")
    ssh = start_sshx(ct)
    log.info(f"[{vps_id}] SSHX link ready: {ssh}")

    return ct, ssh


def start_sshx(ct) -> str:
    """Start an SSHX web terminal and return its shareable URL."""
    # Stop an older SSHX process and clear its log.
    ct.exec_run(
        "bash -c 'pkill -f " + chr(39) + "[s]shx" + chr(39) + " 2>/dev/null || true; "
        "rm -f /tmp/sshx.log /tmp/sshx.pid'",
        tty=False,
    )

    # The official SSHX installer places the sshx binary on the VPS.
    # Run it in the background and capture stdout/stderr for the generated URL.
    result = ct.exec_run(
        "bash -c 'nohup sshx > /tmp/sshx.log 2>&1 & echo $! > /tmp/sshx.pid'",
        tty=False,
    )
    if result.exit_code != 0:
        raise RuntimeError("Failed to start SSHX.")

    for _ in range(45):
        time.sleep(1)
        r = ct.exec_run("bash -c 'cat /tmp/sshx.log 2>/dev/null'", tty=False)
        output = r.output.decode(errors="ignore") if r.output else ""
        match = re.search(r"https://sshx\.io/[^\s\"'<>]+", output)
        if match:
            return match.group(0).rstrip(".,)")

    raise RuntimeError(
        "SSHX did not provide a share link within 45 seconds. "
        "Check /tmp/sshx.log inside the VPS."
    )


def regen_ssh(ct) -> str:
    """Restart SSHX and return a fresh web terminal URL."""
    return start_sshx(ct)
...
@bot.tree.command(name="regen-ssh", description="Get a fresh SSHX web terminal link.")
@app_commands.describe(vps_id="e.g. dracohost-vps-0001")
async def cmd_regen(ix: discord.Interaction, vps_id: str):
    ...
    try:
        ...
        ssh_embed = em(f"🔑 SSH Session — {vps_id}",
            ...
            GREEN, fields=[("🌐 SSHX Link", f"```{ssh}```", False)])
    ...
    async def btn_ssh(self, ix: discord.Interaction, b: discord.ui.Button):
        ...
        try:
            ...
            ssh_embed = em(f"🔑 SSH Session — {self.vps_id}",
                "⚠️ Keep private.", GREEN, fields=[("🌐 SSHX Link", f"```{ssh}```", False)])
            try:
                dm = await ix.user.create_dm()
                await dm.send(embed=ssh_embed)
...
async def cmd_commands(ix: discord.Interaction):
    ...
    u = em("👤 User Commands", "", BLUE, fields=[
        ...
        ("`/regen-ssh <id>`",       "🔑  Fresh SSHX web terminal link",        False),
    ...
    r = em("📖 Reference", "", DARK, fields=[
        ...
        ("SSH Access",  "SSHX web terminal link — sent to DM, never public",                     False),
